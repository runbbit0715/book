...
import { contnets, info, memos } from '../../data/books/themoneybook'
...
const TheMoneyBook = () => {
  ...
  return (
    <Main title='The money book 독서노트' description='토스의 the money book을 읽고 정리한 독서노트입니다.'>
    ...
    <section id="book">
      ...
      <div id='contents'>
        <h2>문장 수집</h2>
      {contnets.map((themoneybook, key) => (
        <p key={key} className='content'>
          <span className='page'>{themoneybook.page}p</span><span className='q'>(Q {themoneybook.q})</span> "{themoneybook.content}"
        </p>
      ))}
      </div>
